...
def _select_available_ids(sample_ids, available_like):
    """Split sample IDs into available and missing lists."""
...
def _safe_torch_load(path, map_location):
    """Load checkpoints across PyTorch versions with different weights_only support."""
    try:
...
def _to_dense_float32(X):
    """Convert sparse or dense matrices into float32 NumPy arrays."""
    if sp.issparse(X):
...
def autocast_context(use_cuda: bool):
    """Use mixed precision on CUDA and a no-op context on CPU."""
    if use_cuda:
...
def _stable_string_seed(text: str) -> int:
    """Derive a deterministic integer seed from text."""
    text = str(text)
    return int(sum((i + 1) * ord(ch) for i, ch in enumerate(text)))


def _clean_path_part(part):
    """Sanitize one path fragment for generated filenames."""
    part = str(part).strip()
    if not part:
        return ""
    for old, new in [
...
def format_p_tag(p_val):
    """Format one corruption probability for filenames."""
...
def build_output_path(save_dir, category, name, ext, split=None, sample_id=None, p_val=None, extra_parts=None):
    """Build one output path from split, sample, and corruption metadata."""
    parts = [category]
...
def _prepare_panel_metadata(sample_id: str, threshold: int, genes_threshold: int):
    """Keep backed access plus filtered panel metadata."""
    ...
    if {"total_counts", "n_genes_by_counts"}.issubset(ad_panel.obs.columns):
        ...
        warnings.warn(
            f"{sample_id}: QC columns missing, computing them chunk-wise from backed X",
            RuntimeWarning,
            stacklevel=2,
        )
...
def _panel_global_bases(panel_data, panel_ids):
    """Build global cell-index offsets for each panel."""
    bases = {}
    offset = 0
...
def compute_panel_present_gene_mask(panel_data, sample_id, common_genes):
    """Mark genes that are present in one panel."""
...
def compute_train_seen_gene_mask(panel_data, train_panel_ids, common_genes):
    """Mark genes seen in the training panels."""
    common_genes = pd.Index(common_genes).astype(str)
    mask = np.zeros(len(common_genes), dtype=bool)
...
def compute_valid_overlap_gene_mask(panel_data, sample_id, common_genes, train_seen_gene_mask):
    """Mark genes that overlap the training gene set."""
...
class VAE(nn.Module):
    def __init__(self, input_dim, latent_dim=16, hidden_dim=256):
        """Create the VAE encoder and decoder modules."""
        super().__init__()
    ...
    def encode(self, x):
        """Encode one batch of cells into latent mean and log-variance."""
    ...
    def decode(self, z):
        """Decode latent vectors back into gene-space logits."""
        h = F.relu(self.dec_fc1(z))
        h = F.relu(self.dec_fc2(h))
        return self.out(h)

    def forward(self, x):
        """Run the VAE and return reconstruction logits plus latent parameters."""
...
def vae_loss(
    ...
    """Compute the VAE loss with ZINB reconstruction and KL regularization."""
...
class PanelRowAccessor:
    def __init__(self, panel_data, panel_ids, common_genes):
        """Store panel indexing tables for the shared gene space."""
        self.panel_defs = []
        self.panel_sizes = []
    ...
    def _locate(self, global_cell_idx):
        """Map a global cell index to a panel index and row index."""
    ...
    def get_row_dense(self, global_cell_idx):
        """Load one cell into the shared dense gene space."""
    ...
    def iter_blocks(self, chunk_size=2048, selected_global_idx=None, split_name_by_sid=None):
        """Yield dense blocks in the shared gene space."""
...
def materialize_panel_in_gene_space(panel_data, sample_id, common_genes, chunk_size=4096):
    """Materialize one panel in the shared gene space."""
...
def corrupt_batch_deterministic(x_clean, global_idx_np, version_idx, p_val, base_seed, exact=True):
    """Apply deterministic corruption for one batch and one p setting."""
...
def sample_zinb_counts_np(mu_nb, theta_vec, pi_vec, rng, eps=1e-8):
    """Sample counts from ZINB parameters with a NumPy RNG."""
...
def zinb_sample_once_batched(model, X_in, log_theta, logit_pi, device, batch_size=1024, rng_seed=0):
    """Draw one batched ZINB reconstruction sample from the VAE."""
    model.eval()
...
def init_metric_acc():
    """Initialize running sums for reconstruction metrics."""
    return {
        "sse_counts": 0.0,
        "sse_log1p": 0.0,
        "sum_true_counts": 0.0,
...
def update_metric_acc(acc, pred, true, gene_mask=None):
    """Accumulate reconstruction metrics for one batch."""
...
def finalize_metric_acc(acc):
    """Convert accumulated sums into final reconstruction metrics."""
    if acc["n_elements"] == 0:
        return {
            "mse_counts": np.nan,
            "rmse_counts": np.nan,
...
def build_scoreboard_from_acc_dict(acc_dict):
    """Build a ranked metric table for each evaluation method."""
    rows = []
...
def rmse_and_r2(pred, true):
    """Compute RMSE and R2 for one prediction target pair."""
...
def save_bar_plot(df, category_col, value_col, title, ylabel, save_path, rotate=True):
    """Save a simple bar plot for one evaluation summary."""
...
def plot_exact_scatter_with_fit(
    ...
    """Plot a scatter with identity and fitted reference lines."""
...
def save_reconstruction_scatter_suite(
    ...
    """Save the mean and detection scatter plots for one reconstruction analysis."""
...
def _find_spatial_xy(adata_obj):
    """Return spatial x/y arrays and the source label used."""
...
def _ensure_spatial_coords(adata):
    """Populate adata.obsm['spatial'] from available coordinate columns."""
...
def zinb_expected_counts_batched(model, X_in, logit_pi, device, batch_size=1024):
    """Run batched VAE inference and return expected decoded counts."""
    model.eval()
    pi_vec = torch.sigmoid(logit_pi.detach()).cpu().numpy().astype(np.float32)
...
def expected_detection_rate_from_zinb_expected_counts(x_expected, theta_vec, pi_vec, eps=1e-8):
    """Compute expected detection rates from decoded ZINB parameters."""
...
def select_high_low_genes(gene_names, mean_target, n_high=4, n_low=4):
    """Select high- and low-expression genes for diagnostics."""
...
def plot_selected_gene_histograms(
    ...
    """Plot histogram comparisons for selected genes."""
...
def plot_selected_gene_spatial_triplets(
    ...
    """Plot spatial input, reconstruction, and target maps for selected genes."""
    x_in, y_in, _ = _find_spatial_xy(adata_input)
    x_tar, y_tar, _ = _find_spatial_xy(adata_target)

    if x_in is None or x_tar is None:
        warnings.warn(
            "Spatial gene plots skipped because no spatial coordinates are available",
            RuntimeWarning,
            stacklevel=2,
        )
        return
...
def run_global_test_evaluation(
    ...
    """Evaluate all test panels and save summary metrics."""
    eval_p_values = np.asarray(full_p_values, dtype=np.float64)
    ...
    method_xp = "Scaled identity baseline (input / p)"
    methods = [method_vae, method_id, method_xp]
    ...
    for version_idx, p_val in enumerate(eval_p_values):
        ...
        for payload in eval_iter:
            ...
            xp_baseline = xb_np / np.float32(max(float(p_val), 1e-8))
            ...
            update_metric_acc(overall_acc[method_xp], xp_baseline, yb_np)
            ...
            update_metric_acc(per_p_acc[(version_idx, method_xp)], xp_baseline, yb_np)
            ...
            if split_name in {"A", "B", "C"}:
                ...
                update_metric_acc(per_split_acc[(version_idx, split_name, method_xp)], xp_baseline, yb_np)
                ...
                update_metric_acc(per_panel_acc[(version_idx, split_name, sample_id, method_xp)], xp_baseline, yb_np)
                ...
                if split_name in {"B", "C"}:
                    ...
                    if np.any(valid_overlap_mask):
                        ...
                        update_metric_acc(
                            per_panel_valid_overlap_acc[(version_idx, split_name, sample_id, method_xp)],
                            xp_baseline,
                            yb_np,
                            gene_mask=valid_overlap_mask,
                        )
...
def run_single_panel_gene_distribution_analysis(
    ...
    """Compare gene distributions for one representative panel."""
    if sample_id not in panel_data:
        warnings.warn(
            f"Split {split_name} skipped because {sample_id} is not present in panel_data",
            RuntimeWarning,
            stacklevel=2,
        )
        return
    ...
    if sample_id not in full_test_bases:
        warnings.warn(
            f"Split {split_name} skipped because {sample_id} is not in the full test ordering",
            RuntimeWarning,
            stacklevel=2,
        )
        return
    ...
    if n_cells == 0:
        warnings.warn(
            f"Split {split_name} skipped because {sample_id} has no cells",
            RuntimeWarning,
            stacklevel=2,
        )
        return
    ...
    if len(selected_genes) == 0:
        warnings.warn(
            f"Split {split_name} skipped because no genes were selected for {sample_id}",
            RuntimeWarning,
            stacklevel=2,
        )
        return
...
def run_split_panel_reconstruction_analysis(
    ...
    """Run reconstruction diagnostics for one representative split panel."""
    if sample_id not in panel_data:
        warnings.warn(
            f"Reconstruction analysis for split {split_name} skipped because {sample_id} is not present in panel_data",
            RuntimeWarning,
            stacklevel=2,
        )
        return
    ...
    if sample_id not in full_test_bases:
        warnings.warn(
            f"Reconstruction analysis for split {split_name} skipped because {sample_id} is not in the full test ordering",
            RuntimeWarning,
            stacklevel=2,
        )
        return
    ...
    if valid_gene_idx.size == 0:
        warnings.warn(
            f"Reconstruction analysis for split {split_name} skipped because {sample_id} has no valid train-overlap genes",
            RuntimeWarning,
            stacklevel=2,
        )
        return
    ...
    if X_clean_full.shape[0] == 0:
        warnings.warn(
            f"Reconstruction analysis for split {split_name} skipped because {sample_id} has no cells",
            RuntimeWarning,
            stacklevel=2,
        )
        return
...
def run_pair_analysis(
    ...
    """Analyze the paired panels used for qualitative comparison."""
...
def main():
    """Coordinate evaluation data loading, model setup, and diagnostics."""
    ...
    requested_ids = sorted(set(train_panel_ids + test_panel_ids + [pair_5k_id, pair_v1_id]))
    ...
    if missing_train:
        warnings.warn(f"Train samples missing from disk: {missing_train}", RuntimeWarning, stacklevel=2)
    if missing_test:
        warnings.warn(f"Test samples missing from disk: {missing_test}", RuntimeWarning, stacklevel=2)

    if load_failed:
        warnings.warn(
            "Failed to load sample metadata: "
            + "; ".join(f"{sid}: {msg}" for sid, msg in load_failed[:10]),
            RuntimeWarning,
            stacklevel=2,
        )
